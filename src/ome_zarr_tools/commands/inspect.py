...
def _level_report(group: zarr.Group, dataset_path: str) -> dict:
    ...
    return {
        ...
        "stored_bytes": int(array.nbytes_stored()),
...
def build_report(zarr_path: str) -> dict:
    ...
    return {
        ...
        "levels": levels,
        "total_stored_bytes": sum(level["stored_bytes"] for level in levels),
...
def _format_text(report: dict) -> str:
    ...
    for level in report["levels"]:
        ...
        lines.append(f"  stored size:  {level['stored_bytes']} bytes")
        lines.append(f"  logical size: {level['logical_bytes']} bytes")
        lines.append("")
    lines.append(f"Total stored size:  {report['total_stored_bytes']} bytes")
